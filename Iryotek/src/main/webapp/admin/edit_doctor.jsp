						...
						<form action="../updateDoctor" method="post">
							...
							<div class="mb-3">
								<label class="form-label">Hospital</label> <select
									class="form-control" name="hospitalId" required="required">
									<option disabled="disabled">---Select Hospital---</option>

									<%
									HospitalDAO hospDAO = new HospitalDAO(DBConnection.getConn());
									List<Hospital> hospList = hospDAO.getAllHospital();
									for (Hospital h : hospList) {
										String selected = (h.getId() == doctor.getHospitalId()) ? "selected" : "";
									%>
									<option value="<%=h.getId()%>" <%=selected%>>
										<%=h.getName()%> (<%=h.getCity()%>)
									</option>
									<%
									}
